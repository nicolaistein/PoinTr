    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if (
                ...
                or dataset_name == "Projected_ShapeNet"
                or dataset_name == "ModelNet"
            ):
                partial = data[0].cuda()
                gt = data[1].cuda()
                if config.dataset.train._base_.CARS:
                    ...
                    partial = misc.random_dropping(
                        partial, epoch
                    )  # specially for KITTI finetune
            ...
            if train_writer is not None:
                train_writer.add_scalar(
                    "Loss/Batch/Sparse", sparse_loss.item() * 1000, n_itr
                )
                train_writer.add_scalar(
...
def test(
    ...
    with torch.no_grad():
        ...
        for _, v in category_metrics.items():
            test_metrics.update(v.avg())
        print_log(
            "[TEST] Metrics = %s" % (["%.4f" % m for m in test_metrics.avg()]),
            logger=logger,
        )
